...
setwd("C:/Users/ADMIN/Desktop/Introduction to data science")
library(readxl)
Loan_Approval_Data <- read_excel("Loan_Approval_Data.xlsx")
Data <- Loan_Approval_Data
View(Data)

#1.Exploring the dataset and identifying the datatypes of each variable
#To view the data types of each variable
sapply(Loan_Approval_Data,class)

#View datatypes again 
sapply(Data,class)


#2.Transforming the dataset and removing missing data and outliers if any
sum(is.na(Data))
print(Data)
#The missing data is 150
print("After removing the NA values ")
Data_omit=na.omit(Data)
print(Data_omit)

#Finding outliers
Outliers_AI <- boxplot.stats(Data$ApplicantIncome)$out
Outliers_CI <- boxplot.stats(Data$CoapplicantIncome)$out
Outliers_LA <- boxplot.stats(Data$LoanAmount)$out
Outliers_LAT <- boxplot.stats(Data$Loan_Amount_Term)$out
Outliers_CH <- boxplot.stats(Data$Credit_History)$out
...
Data_new = remove_outlier(Data_omit, c('ApplicantIncome', 'CoapplicantIncome', 'LoanAmount', 'Loan_Amount_Term', 'Credit_History'))

#D. Save the transformed dataset
install.packages("WriteXLS")
library(WriteXLS)
WriteXLS(Data_new, ExcelFileName = "Loan_Approval_Data.xlsx",
         SheetNames = "LoanData",
         AdjWidth = T,
         BoldHeaderRow = T)
...
cor.test(Data$ApplicantIncome, Data$LoanAmount, use="complete.obs")
#There is a moderate correlation of 0.570909
#Visuals using scatterplot
library(ggplot2)
ggplot(data = Data_new, mapping = aes(x = ApplicantIncome, y = LoanAmount)) + 
  geom_point() + 
  labs(x="ApplicantIncome", 
       y="LoanAmount",
       title="Loan amounts with varying applicant incomes")

ggplot(data = Data, mapping = aes(x = ApplicantIncome, y = LoanAmount)) + 
  geom_point() +
  labs(x="ApplicantIncome", 
       y="LoanAmount",
       title="Loan amounts with varying applicant incomes")
...
#visuals using boxplot
ggplot(data = Data_new, mapping = aes(x = Education, y = ApplicantIncome)) +
  geom_boxplot() +
  labs(x="Education",   
       y="ApplicantIncome",
       title="How education contibutes to applicant income")

ggplot(data = Data, mapping = aes(x = Education, y = ApplicantIncome)) +
  geom_boxplot() +
  labs(x="Education", 
       y="ApplicantIncome",
       title="How education contibutes to applicant income")
...
ggplot(data = Data_new) +
  geom_count(mapping = aes(x = Gender, y = Dependents)) +
  labs(x="Gender", 
       y="Dependents",
       title="How dependents vary with gender")
...
library(readxl)
Bike_Sales_New <- read_excel("Bike_Sales_New.xlsx")
View(Bike_Sales_New)
Data = readxl::read_xlsx("Bike_Sales_New.xlsx")
